                ...
                <div class="col-sm-12">
                    <div class="category-description std">
                        <div class="slider-items-products">
                            <h2 class="page-heading"><span class="page-heading-title">{{ trans('designer.send_to_review.title') }}</span>
                            </h2>
                    ...
                    <div>
                        ...
                        <form class="form-list" role="form" method="POST" action="{{ route('designer.sendDesignToReview', $design->getId()) }}">
                            {{ csrf_field() }}

                            <div class="col-md-12 form-group{{ $errors->has('name') ? ' has-error' : '' }}">
                                <label for="name" class="control-label">{{ trans('designer.send_to_review.name') }}</label>
                            ...
                            <div class="col-md-12 form-group{{ $errors->has('description') ? ' has-error' : '' }}">
                                <label for="description" class="control-label">{{ trans('designer.send_to_review.description') }}</label>

                                <textarea id="description" type="text" class="form-control" name="description"
                                          required placeholder="{{ trans('designer.send_to_review.description_placeholder') }}" rows="4">{{ old('description') }}</textarea>
                            ...
                            <div class="col-md-12 form-group{{ $errors->has('occasions') ? ' has-error' : '' }}">
                                <label for="inputOccasion" class="control-label">{{ trans('designer.send_to_review.occasions') }}</label>

                                <select id="inputOccasion" name="occasions[]" multiple="multiple" class="form-control">
                                    <option></option>
                                    @foreach($occasions as $occasion)
                                        <option value="{{ $occasion->getId() }}">{{ $occasion->getParent()->getName() }} - {{ $occasion->getName() }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-12 form-group{{ $errors->has('commission') ? ' has-error' : '' }}">
                                <label for="commission" class="control-label">{{ trans('designer.send_to_review.commission') }}: </label>

                                <input type="text" value="5%" id="commission-value" readonly
                                       style="border:0; color:#f6931f; font-weight:bold;">

                                <input id="commission" type="text" name="commission" class="form-control" value="" data-slider-min="5"
                            ...
                            <div class="col-md-12">
                                <p><a href="{{ route('terms_and_conditions') }}">{{ trans('designer.send_to_review.accept_terms') }}</a></p>
                            </div>
